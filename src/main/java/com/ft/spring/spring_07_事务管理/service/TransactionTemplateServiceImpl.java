...
@Service
@Order(3)
public class TransactionTemplateServiceImpl implements InsertService{
    @Autowired
    ...
    public void insert() {
        ...
        Object o = transactionTemplate.execute((TransactionCallback) status -> {
            try {
                // 执行数据库操作
                curdService.insert("TransactionTemplate", "编程式-基于事务模板-方式1");
        ...
        Object o2 = transactionTemplate.execute((TransactionCallback) status -> {
            // 执行数据库操作
            curdService.insert("TransactionTemplate", "编程式-基于事务模板-方式2");
